...
{
    public class DUpdateTaskCollection : IUpdateTaskCollection
    {
        private Inventory360Entities _db;
        private Task_Collection _findEntity;

        public DUpdateTaskCollection(Guid id)
        {
            _db = new Inventory360Entities();
            _db.Configuration.LazyLoadingEnabled = false;

            // Initialize value
            _findEntity = _db.Task_Collection.Find(id);
        }

        [OperationBehavior(TransactionScopeRequired = true, TransactionAutoComplete = true)]
        [TransactionFlow(TransactionFlowOption.Allowed)]
        public bool UpdateCollectionForApprove(long approvedBy)
        {
            try
            {
        ...
        [OperationBehavior(TransactionScopeRequired = true, TransactionAutoComplete = true)]
        [TransactionFlow(TransactionFlowOption.Allowed)]
        public bool UpdateCollectionForCancel(string reason, long cancelledBy)
        {
            try
            {
