myApp
    .controller('complainReceiveAnalysisReportController', ['$scope', '$q', 'applicationBasePath', 'userService',
        ...
        'productCommonService', 'customerCommonService', 'complainReceiveCommonService', 'complainReceiveAnalysisReportNameCommonService',
        function (
            ...
            $scope.selectedReceiveId = "";
            $scope.selectedCustomerId = 0;
            $scope.selectedProductId = 0;

            $scope.getComplainReceiveNo = function () {
                var q = $scope.complainReceiveNo === undefined ? '' : $scope.complainReceiveNo;

                if (q.length >= 3) {
                    complainReceiveCommonService.FetchComplainReceiveNo(
                        q
                    ).then(function (value) {
            ...
            $scope.fillComplainReceiveNoTextbox = function (obj) {
                $scope.selectedReceiveId = obj.Value;
            ...
            $scope.PrintReport = function () {
                var url = applicationBasePath + "/Inventory360Reports/PrintComplainReceiveAnalysis?reportName=" + $scope.reportName
                    + "&locationId=" + $scope.locationId
                    + "&dateFrom=" + ($scope.dateFrom == undefined ? '' : $scope.dateFrom)
                    + "&dateTo=" + ($scope.dateTo == undefined ? '' : $scope.dateTo)
                    + "&complainReceiveId=" + ($scope.selectedReceiveId == undefined ? '' : $scope.selectedReceiveId)
            ...
            var GetReportNameForDrowdown = function () {
                complainReceiveAnalysisReportNameCommonService.FetchSalesReportName().then(function (value) {
